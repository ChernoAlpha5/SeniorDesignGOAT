...
void setup()
{
  ...
  */
  ...
  baseValue /= avgAmount;*/
  //1000000 = 1 second
    Timer1.initialize(1000000);   //interrupt every 1 seconds
...
void loop()
{
  ...
  if (state){
    if (state == 1){
      ...
      //Serial.println("0");
      float bpm = getBPM();
      if (cnt == 6){ //output once every 6 runs because original sampling rate is too fast for smartphone
        Serial.print(bpm);
        Serial.print(" ");
        Serial.println(irValue); //Send raw data
        cnt = 0;
      }
      cnt++;
